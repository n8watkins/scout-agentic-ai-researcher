...
export function PoolMeterBar({ usage, variant = 'light' }: PoolMeterBarProps) {
  ...
  const subClass = light
    ...
    : 'text-violet-600 dark:text-violet-300/80';
...
export default function SidebarUsageMeter({ hasOwnKey }: SidebarUsageMeterProps) {
  ...
  return (
    <div
      className="px-3 py-2 bg-violet-100 dark:bg-violet-900/30 rounded-lg border border-violet-200 dark:border-violet-700/40"
      data-testid="sidebar-usage-meter"
    >
      {hasOwnKey ? (
        <div>
          <div className="flex items-center justify-between text-xs mb-1">
            <span className="font-semibold text-violet-800 dark:text-violet-200">Your API key</span>
            <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
              <span className="w-1.5 h-1.5 bg-green-500 dark:bg-green-400 rounded-full" />
              active
            </span>
          </div>
          <p className="text-xs text-violet-600 dark:text-violet-300/80">Unlimited research on your own key</p>
        </div>
      ) : (
